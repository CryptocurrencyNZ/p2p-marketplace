  ...
  useEffect(() => {
    const initializeMap = async () => {
      ...
      if (mapRef.current && !mapRef.current.querySelector('.mapboxgl-canvas-container')) {
        ...
        mapboxgl.accessToken = 'pk.eyJ1IjoiamF5ZGVuLWNueiIsImEiOiJjbThpZTljaW8wNmh0MmtvZTh4czRycWp1In0.P3UGPsCBIDnPIADynqDMrw'; // Replace with your token
    ...
    <div className={`${className} relative`}>
      ...
      <div className="flex flex-col md:flex-row md:h-[600px]">
        ...
        <div 
          className={`
            ${isMobile ? 
              (viewMode === 'map' ? 'block h-[50vh] min-h-[400px]' : 'hidden') : 
              'flex-grow h-full'
        ...
        <div 
          className={`
            ${isMobile ?
              (viewMode === 'listings' ? 'block h-[50vh] min-h-[400px]' : 'hidden') :
